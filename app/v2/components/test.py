...
class DatasetInputComponent:
    def __init__(
        self, root_frame: ttk.Frame, column: int = 0, row: int = 0, data_service=None
    ):
    ...
    def _add_target(self, _=None):
        current_selection = self.input_list.curselection()
        if current_selection != () and self.target_list.size() < 1:
            a = self.input_list.get(current_selection)
    ...
    def get_params(self) -> dict:
        return {
            "predictor_names": self.get_predictors(),
    ...
    def attach_frame(
        self, root_frame: ttk.Frame, column: int, row: int
    ) -> ttk.Labelframe:
        # Get Train Set
        ...
        ttk.Entry(train_set_frame, textvariable=self.train_file_path_var).grid(
            column=1, row=0
        )
        ttk.Button(train_set_frame, text="Read Data", command=self._read_data).grid(
            column=2, row=0
        )
    ...
    def attach_input_list(self, root_frame: Union[ttk.Frame, ttk.LabelFrame]):
        ...
        ttk.Button(root_frame, text="Add Predictor", command=self._add_predictor).grid(
            column=1, row=2
        )
        ttk.Button(
            root_frame, text="Eject Predictor", command=self._eject_predictor
        ).grid(column=1, row=3)
        ttk.Button(
            root_frame, text="Debug", command=lambda: print(self.input_list.get(0))
        ).grid(column=0, row=2)
        ttk.Button(
            root_frame,
            text="Debug 2",
            command=lambda: print(self.input_list.curselection()),
        ).grid(column=0, row=3)

        ttk.Button(root_frame, text="Add Target", command=self._add_target).grid(
            column=2, row=2
        )
        ttk.Button(root_frame, text="Eject Target", command=self._eject_target).grid(
            column=2, row=3
        )
...
class ModelValidationComponent:
    ...
    def _open_entries(self):
        if self.do_forecast_option_var.get():
            self.entry_vars[0]["state"] = (
                tk.NORMAL if self.validation_option_var.get() else tk.DISABLED
            )
            for radiobutton in self.radio_vars:
                radiobutton["state"] = tk.DISABLED
        else:
            self.entry_vars[0]["state"] = (
                tk.NORMAL if self.validation_option_var.get() == 1 else tk.DISABLED
            )
            self.entry_vars[1]["state"] = (
                tk.NORMAL if self.validation_option_var.get() == 2 else tk.DISABLED
            )
            for radiobutton in self.radio_vars:
    ...
    def get_params(self) -> dict:
        return {
            ...
            if self.validation_option_var.get() == 1
            else None,
            "k_fold_cv": self.k_fold_value_var.get()
            if self.validation_option_var.get() == 2
            else None,
        }
    ...
    def attach_frame(self, root_frame: ttk.Frame, column: int, row: int):
        ...
        model_validation_frame = ttk.Labelframe(
            root_frame, text="Model testing and validation"
        )
        ...
        tk.Checkbutton(
            ...
            offvalue=0,
            onvalue=1,
            variable=self.do_forecast_option_var,
            command=self._open_entries,
        ...
        tk.Radiobutton(
            ...
            value=0,
            variable=self.validation_option_var,
            command=self._open_entries,
        ...
        leave_one_out_radio = tk.Radiobutton(
            model_validation_frame,
        ...
        k_fold_value_entry = ttk.Entry(
            model_validation_frame, textvariable=self.k_fold_value_var, width=8
        )
...
class CustomizeTrainsetComponent:
    ...
    def attach_frame(self, root_frame: ttk.Frame, column: int, row: int):
        # Customize Train Set
        self.customize_train_set_frame = ttk.LabelFrame(
            root_frame, text="Customize Train Set"
        )
        self.customize_train_set_frame.grid(column=column, row=row)

        self.scale_var = tk.StringVar(value="None")
        ttk.Label(self.customize_train_set_frame, text="Scale Type").grid(
            column=0, row=0
        )
        ttk.OptionMenu(
            self.customize_train_set_frame,
            self.scale_var,
            "None",
            "None",
            "StandardScaler",
    ...
    def _open_lookback_entries(self):
        self.entry_vars[0]["state"] = (
            tk.NORMAL if self.lookback_option_var.get() else tk.DISABLED
        )
        self.entry_vars[1]["state"] = (
            tk.NORMAL if self.seasonal_lookback_option_var.get() else tk.DISABLED
        )
        self.entry_vars[2]["state"] = (
            tk.NORMAL if self.seasonal_lookback_option_var.get() else tk.DISABLED
        )

    def get_sliding(self) -> int:
        return (
            self.lookback_option_var.get()
            + 2 * self.seasonal_lookback_option_var.get()
            - 1
        )
    ...
    def get_params(self) -> dict:
        ...
        if self.lookback_attached:
            lookback_params = {
                ...
                "lookback_value": self.lookback_value_var.get()
                if self.lookback_option_var.get()
                else None,
                "seasonal_lookback_option": self.seasonal_lookback_option_var.get(),
                "seasonal_period": self.seasonal_period_value_var.get()
                if self.seasonal_lookback_option_var.get()
                else None,
                "seasonal_value": self.seasonal_value_var.get()
                if self.seasonal_lookback_option_var.get()
                else None,
            }
    ...
    def check_errors(self) -> Tuple[bool, str]:
        ...
        if self.seasonal_lookback_option_var.get():
            if (
                not self.seasonal_value_var.get()
                or not self.seasonal_period_value_var.get()
            ):
                return (True, "Select predictors")
        return (False, "")
    ...
    def attach_lookback(self):
        ...
        tk.Checkbutton(
            ...
            offvalue=0,
            onvalue=1,
            variable=self.lookback_option_var,
            command=self._open_lookback_entries,
        ).grid(column=0, row=1)

        lookback_entry = tk.Entry(
            self.customize_train_set_frame,
            textvariable=self.lookback_value_var,
            width=8,
            state=tk.DISABLED,
        )
        ...
        tk.Checkbutton(
            ...
            offvalue=0,
            onvalue=1,
            variable=self.seasonal_lookback_option_var,
            command=self._open_lookback_entries,
        ).grid(column=0, row=2)

        seasonal_lookback_entry_1 = tk.Entry(
            self.customize_train_set_frame,
            textvariable=self.seasonal_period_value_var,
            width=9,
            state=tk.DISABLED,
        )
        seasonal_lookback_entry_1.grid(column=0, row=3)
        seasonal_lookback_entry_2 = tk.Entry(
            self.customize_train_set_frame,
            textvariable=self.seasonal_value_var,
            width=8,
            state=tk.DISABLED,
        )
        ...
        self.entry_vars = [
            lookback_entry,
            seasonal_lookback_entry_1,
            seasonal_lookback_entry_2,
        ]
...
class TestModelComponent:
    def __init__(
        self, root_frame: ttk.Frame, column: int = 1, row: int = 1, **functions
    ):
    ...
    def attach_frame(self, root_frame: ttk.Frame, column: int, row: int):
        ...
        ttk.Entry(test_model_main_frame, textvariable=self.forecast_num_var).grid(
            column=1, row=0
        )
        ttk.Button(
            test_model_main_frame,
            text="Values",
            command=self.show_predict_values_function,
        ).grid(column=2, row=0)

        self.test_file_path_var = tk.StringVar()
        ttk.Label(test_model_main_frame, text="Test File Path").grid(column=0, row=1)
        ttk.Entry(test_model_main_frame, textvariable=self.test_file_path_var).grid(
            column=1, row=1
        )
        ttk.Button(
            test_model_main_frame, text="Get Test Set", command=self._read_test_set
        ).grid(column=2, row=1)

        ttk.Button(
            test_model_main_frame, text="Test Model", command=self.forecast_function
        ).grid(column=2, row=3)
        ttk.Button(
            test_model_main_frame,
            text="Actual vs Forecast Graph",
            command=self.graph_predicts_function,
