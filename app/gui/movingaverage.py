...
class MovingAverage:
    ...
    def __init__(self):
        ...
        ttk.Button(train_frame,
                   ...
                   command=lambda: self.read_train_data(file_path)).grid(
                       column=2, row=0)
        ...
        ttk.Entry(train_frame, textvariable=self.period_count).grid(column=1,
                                                                    row=5)
        ...
        ttk.Entry(test_frame, textvariable=self.forecast_num).grid(column=1,
                                                                   row=0)
        ...
        ttk.Button(test_frame, text="Create Model",
                   command=self.create_model).grid(column=0, row=2)
        ttk.Button(test_frame, text="Graph Data",
                   command=self.graph_data).grid(column=1, row=2)
        ttk.Button(test_frame, text="Show Data",
                   command=self.show_data).grid(column=2, row=2)
    ...
    def graph_data(self):
        if self.do_test:
            y_test = self.test_df[
                self.col].iloc[:self.forecast_num.get()].values  # type: ignore
    ...
    def create_model(self):
        ...
        for _ in range(self.forecast_num.get() // period + 1):
            val = val[-train_size:]
            out = np.array([val[i::period].mean()
                            for i in range(period)])  # type: ignore
