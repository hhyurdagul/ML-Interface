...
def NMSE(y_true, y_pred):
    return np.round((((y_true-y_pred)**2)/(y_true.mean()*y_pred.mean())).mean(), 2)

def RMSE(y_true, y_pred):
    return np.round(np.sqrt(mean_squared_error(y_true, y_pred)), 2)

def MAE(y_true, y_pred):
    return np.round(mean_absolute_error(y_true, y_pred), 2)
...
def SMAPE(y_true, y_pred):
    try:
        return np.round(np.mean(np.abs(y_true-y_pred)/((np.abs(y_true)+np.abs(y_pred))/2)) * 100, 2)
    except:
        return None

def MASE(y_true, y_pred, seasons=1):
    try:
        return np.round(mean_absolute_error(y_true, y_pred) / mean_absolute_error(y_true[seasons:], y_true[:-seasons]), 2)
    except:
        return None
...
def loss(y_true, y_pred, seasons=1):
    ...
    NMSE = np.round((((y_true-y_pred)**2)/(y_true.mean()*y_pred.mean())).mean(), 2)
    ...
    try:
        ...
        MAPE = None 
    try:
        SMAPE = np.round(np.mean(np.abs(y_true-y_pred)/((np.abs(y_true)+np.abs(y_pred))/2)) * 100, 2)
    except:
        SMAPE = None
    try:
        MASE = np.round(mean_absolute_error(y_true, y_pred) / mean_absolute_error(y_true[seasons:], y_true[:-seasons]), 2)
    except:
        MASE = None
