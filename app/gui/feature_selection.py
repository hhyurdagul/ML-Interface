...
class FS:
    ...
    def __init__(self):
        ...
        ttk.Button(get_train_set_frame,
                   text="Read Data",
                   command=lambda: self.read_csv(file_path)).grid(column=2,
                                                                  row=0)
        ...
        # Feature Selection
        feature_selection_frame = ttk.Labelframe(self.root,
                                                 text="Feature Selection")
        feature_selection_frame.grid(column=0, row=1)

        feature_count = tk.IntVar(value=5)
        ttk.Label(feature_selection_frame,
                  text="# Features to select").grid(column=0, row=0)
        ttk.Entry(feature_selection_frame,
                  textvariable=feature_count).grid(column=1, row=0)
        ttk.Button(feature_selection_frame,
                   text="mRMR",
                   command=lambda: self.mrmr_select(feature_count)).grid(
                       column=0, row=1)
        ttk.Button(
            feature_selection_frame,
            text="F-Regression",
            command=lambda: self.f_regression_select(feature_count)).grid(
                column=1, row=1)
        ttk.Button(feature_selection_frame,
                   text="Forward Sequential Selector",
                   command=lambda: self.forward_sequential_select(
                       feature_count)).grid(column=0, row=2)
        ttk.Button(feature_selection_frame,
                   text="Backward Sequential Selector",
                   command=lambda: self.backward_sequential_select(
                       feature_count)).grid(column=1, row=2)
    ...
    def mrmr_select(self, K):
        ...
        df = pd.DataFrame(index=range(len(selected_features)),
    ...
    def f_regression_select(self, K):
        ...
        selected_features = X.columns[selected_features.argsort()
                                      [::-1]].values[:K.get()]
        top = tk.Toplevel(self.root)
        df = pd.DataFrame(index=range(len(selected_features)),
                          data=selected_features)
