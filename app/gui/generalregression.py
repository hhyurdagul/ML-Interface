...
from .helpers import loss, skloss
...
class GeneralRegressionNeuralNetwork:
    ...
    def __init__(self):
        ...
        ttk.Button(get_train_set_frame,
                   text="Read Train Set",
                   command=lambda: self.read_train_data(file_path)).grid(column=2,
                                                                 row=0)
        ...
        ttk.Button(sigma_options_frame,
                   text="Create Model",
                   command=self.create_model).grid(column=0,
                                                  row=5,
                                                  columnspan=2)
    ...
    def forecast(self, num):
        lookback_option = self.lookback_option.get()
        X_test = self.test_df[list(self.predictor_list.get(
            0, tk.END))].iloc[:num].to_numpy()
        y_test = self.test_df[self.target_list.get(
            0)][:num].to_numpy().reshape(-1)
        self.y_test = y_test
            ...
            for i in range(num):
                ...
                if self.scale_var.get() != "None":
                    X_test.iloc[:] = self.feature_scaler.transform(
                        X_test.values.reshape(1, -1)).reshape(-1)
        ...
        if self.scale_var.get() != "None":
            ...
            self.pred = self.label_scaler.inverse_transform(
                self.pred.reshape(-1, 1)).reshape(-1)
