...
class SupportVectorMachine:
    def __init__(self):
        ...
        self.parameters = [
            tk.Variable(value="0.1"),
            tk.Variable(value="0.5"),
            tk.Variable(value="1"),
            tk.Variable(value="1"),
            tk.Variable(value="0"),
            tk.Variable(value="3"),
        ...
        ttk.Label(model_parameters_frame, text="Gamma:").grid(
        ...
        self.gamma_radios = [
            tk.Radiobutton(
                ...
                text=j,
                value=i,
                variable=self.gamma_choice,
                state=tk.DISABLED,
                command=self.__open_entries,
            )
            for i, j in enumerate(["Scale", "Auto", "Value"])
        ]
    ...
    def load_model(self):
        ...
        if self.validation_option.get() == 1:
            self.random_percent_var.set(params.get("random_percent", 0))
    ...
    def create_model(self):
        ...
        if self.grid_option_var.get() == 0:
            ...
            if self.model_type_var.get() == 0:
                ...
                model = NuSVR(
                    kernel=kernel, C=C, nu=nu, gamma=gamma, coef0=coef0, degree=degree
                )
            ...
            params["C"] = np.unique(
                np.logspace(
                    float(self.optimization_parameters[2][0].get()),
                    float(self.optimization_parameters[2][1].get()),
                    interval,
                    base=2,
                )
            )
            if self.model_type_var.get() == 0:
                params["epsilon"] = np.unique(
                    np.linspace(
                        float(self.optimization_parameters[0][0].get()),
                        float(self.optimization_parameters[0][1].get()),
                        interval,
                    )
                )
                model = SVR()
            ...
            if kernel != "linear":
                if gamma_choice == 2:
                    params["gamma"] = np.unique(
                        np.logspace(
                            float(self.optimization_parameters[3][0].get()),
                            float(self.optimization_parameters[3][1].get()),
                            interval,
                            base=2,
                        )
                    )
            ...
            if kernel == "poly" or kernel == "sigmoid":
                params["coef0"] = np.unique(
                    np.linspace(
                        float(self.optimization_parameters[4][0].get()),
                        float(self.optimization_parameters[4][1].get()),
                        interval,
                    )
                )

            if kernel == "poly":
                params["degree"] = np.unique(
                    np.linspace(
                        float(self.optimization_parameters[5][0].get()),
                        float(self.optimization_parameters[5][1].get()),
                        interval,
                        dtype=int,
                    )
                )
