...
class Ridge:
    ...
    def __init__(self):
        ...
        self.lookback_entry = tk.Entry(customize_train_set_frame,
        ...
        tk.Checkbutton(parameter_optimization_frame,
        ...
        self.interval_entry = ttk.Entry(parameter_optimization_frame,
        ...
        tk.Checkbutton(parameter_optimization_frame,
                       ...
                       command=self.open_entries).grid(column=0, row=2)
        self.gs_cross_val_entry = tk.Entry(parameter_optimization_frame,
        ...
        parameter_names = ["Alpha (C)"]
        self.parameters = [tk.DoubleVar(value=1.0)]
        ...
        self.model_parameters_frame_options = [[
            ttk.Label(model_parameters_frame, text=j + ":").grid(column=0,
                                                                 row=i + 1),
            ttk.Entry(model_parameters_frame,
    ...
    def create_model(self):
        ...
        if self.grid_option_var.get() == 0:
            ...
            params["alpha"] = np.unique(
                np.linspace(self.optimization_parameters[0][0].get(),
                            self.optimization_parameters[0][1].get(),
                            interval,
                            dtype=float))

            cv = self.gs_cross_val_var.get() if self.gs_cross_val_option.get(
            ) == 1 else None
            regressor = GridSearchCV(RidgeRegression(), params, cv=cv)
