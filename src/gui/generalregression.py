...
class GeneralizedRegressionNeuralNetwork:
    ...
    def forecast(self, num):
        ...
        for _ in range(num):
            ...
            df = pd.DataFrame([{"holiday": isHoliday(last_t), "weekend": isWeekend(last_t), "hour": last_t.hour, "day": last_t.day, "month": last_t.month, "year":last_t.year}])
