...
class CatBoost:
    ...
    def get_data(self):
        ...
        if scale_choice == "StandardScaler":
            ...
            y.iloc[:] = self.label_scaler.fit_transform(y.to_numpy().reshape(
                -1, 1)).reshape(-1)
    ...
    def create_model(self):
        ...
        if self.grid_option_var.get() == 0:
            max_depth = self.parameters[0].get()
            ...
            regressor = GridSearchCV(
                CatBoostRegressor(allow_writing_files=False), params, cv=cv)
            ...
            for i in range(num):
                ...
                if self.scale_var.get() != "None":
                    X_test.iloc[:] = self.feature_scaler.transform(
                        X_test.to_numpy().reshape(1, -1)).reshape(-1)
