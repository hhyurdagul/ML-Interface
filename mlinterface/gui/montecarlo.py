...
class MonteCarlo:
    def __init__(self):
        ...
        ttk.Button(
            train_frame, textvariable=self.col_var, command=self.select_column
        ).grid(column=2, row=1)
        ...
        ttk.Button(train_frame, text="Create Model", command=self.create_model).grid(
            column=0, row=6
        )
        ...
        ttk.Button(value_frame, text="Create Values", command=self.create_values).grid(
            column=2, row=0
        )
        ttk.Button(value_frame, text="Show Values", command=self.show_values).grid(
            column=2, row=1
        )
        ttk.Button(value_frame, text="Graph Values", command=self.graph_values).grid(
            column=2, row=2
        )
        ...
        ttk.Button(test_frame, text="Test Model", command=self.test_model).grid(
            column=0, row=1
        )
        ttk.Button(test_frame, text="Show Test", command=self.show_test).grid(
            column=1, row=1
        )
        ttk.Button(test_frame, text="Graph Test", command=self.graph_test).grid(
            column=2, row=1
        )
        ttk.Button(test_frame, text="Show Maes", command=self.show_maes).grid(
            column=0, row=2
        )
        ...
        for i, j in enumerate(test_metrics):
            ...
            ttk.Entry(test_frame, textvariable=self.test_metrics_vars[i], width=8).grid(
                column=4, row=i
            )
    ...
    def show_values(self):
        ...
        df = pd.DataFrame(
            {j: self.pred[i * p : (i + 1) * p] for i, j in enumerate(self.day_names)}
        )  # type: ignore
    ...
    def show_maes(self):
        ...
        top = tk.Toplevel(self.root)
        maes = [
            mae(self.y_test[i * p : (i + 1) * p], self.pred[i * p : (i + 1) * p])
            for i in range(c)
        ]
    ...
    def create_model(self):
        ...
        self.period = period
        train = self.df[self.col].iloc[: -self.train_size.get() * period].copy()
        test = self.df[self.col].iloc[-self.train_size.get() * period :].copy()

        for i in range(len(train) // period_size):
            s = np.sum(train[i * period_size : (i + 1) * period_size]) / 100
            train.iloc[i * period_size : (i + 1) * period_size] = np.round(
                train.iloc[i * period_size : (i + 1) * period_size] / s, 3
            )

        arr = np.array([train.iloc[i::period].values for i in range(period)])
        arrs = np.array(
            [arr[i * period_size : (i + 1) * period_size] for i in range(period_count)]
        )
        ...
        for i in arrs:
            ...
            for j in i:
                u.append(np.mean(j))
                s.append(np.std(j))
                r.append(norm.ppf(np.random.rand(1, self.iteration_size.get())))
            us.append(u)
            stds.append(s)
            randoms.append(r)
        ...
        for i in range(self.train_size.get()):
            ...
            for j in range(period_count):
                ...
                best = np.argmin(
                    mae(
                        z.T,
                        np.round(results[j] * np.sum(z[0]) / 100),
        ...
        best_freqs = np.array(
            [[results[i, :, j] for i, j in enumerate(k)] for k in all_maes]
        )
    ...
    def create_values(self):
        list_ = []
        for i, j in enumerate(self.days):
            list_.extend(np.round(self.freqs[i] * j.get() / 100))
        self.pred = np.array(list_)
